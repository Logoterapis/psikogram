      ...
      >
        ...
        <div className="print-header hidden">
          ...
          <span style={{ fontSize: '7px', fontWeight: 700, color: '#64748b' }}>
            {data.identity?.name || '-'} — {data.identity?.position || '-'}
          </span>
        </div>
        ...
        <div className="print-footer hidden">
          <span style={{ fontSize: '7px', fontWeight: 600, color: '#94a3b8', letterSpacing: '0.15em', textTransform: 'uppercase' }}>
            Confidential • Sika Indonesia HR Assessment • {new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
          </span>
        </div>
        {/* Header */}
            ...
            <table className="w-full text-[10px] border-collapse">
              <thead>
                <tr className="bg-slate-100 text-slate-600 uppercase font-bold">
                  ...
                  <th className="p-2 text-center border-b border-slate-200">Skor (Ideal)</th>
                ...
                {(data.papi || []).map((p, i) => (
                  <tr key={i} className="hover:bg-[#fbfcfd] transition">
                    ...
                    <td className="p-2 text-center">
                      ...
                      <div className="text-[9px] font-bold text-slate-400 mt-0.5">TARGET: {p.ideal_score || '-'}</div>
                    </td>
